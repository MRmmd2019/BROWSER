(function ($) {
	...
	// Function to initialize manga slider
	function initMangaSlider($container) {
		if ($container.length === 0 || $container.hasClass('slick-initialized')) {
			return;
		}

		var $parent = $container.parents(".manga-slider");
		...
		var check_style = $container.parents(".style-3").length;
		var check_rtl = (jQuery("body").css('direction') === "rtl");
		var manga_style_1 = {
		...
		switch (style) {
			case 'style-1':
				$container.slick(manga_style_1);
				break;
			case 'style-2':
				$container.slick(manga_style_2);
				break;
			case 'style-3':
			case 'style-4':
				$container.slick(manga_style_3);
				break;
		}
	}

	// Function to refresh slider (recalculate dimensions)
	function refreshMangaSlider($container) {
		if ($container.length === 0 || !$container.hasClass('slick-initialized')) {
			return;
		}
		setTimeout(function() {
	...
	$('.shortcode-wp-manga-tabs').each(function() {
		var $this = $(this);
		$this.find('.tab-content-item:first-child').addClass('active');
		$this.find('.tabs-header .tab-header-item a').on('click', function(e) {
			var $tab = $(this).closest('.shortcode-wp-manga-tabs');
			var $target = $(this).attr('href').replace('#', '');
			$tab.find('.tab-header-item').removeClass('active');
			$(this).parent().addClass('active');
			
			$tab.find('.tab-content-item').removeClass('active');
			var $activeTab = $tab.find('.tab-content-item[data-tab="' + $target + '"]');
			$activeTab.addClass('active');

			// Initialize or refresh sliders in the active tab
			setTimeout(function() {
				$activeTab.find(".manga-slider .slider__container").each(function() {
					var $sliderContainer = $(this);
					if (!$sliderContainer.hasClass('slick-initialized')) {
						initMangaSlider($sliderContainer);
					} else {
						refreshMangaSlider($sliderContainer);
					}
				});
			}, 50);
	...
	// Initialize visible sliders on page load
	$(".manga-slider .slider__container").each(function () {
		var $this = $(this);
		var $tabContent = $this.closest('.tab-content-item');
		
		// Only initialize if slider is in active tab or not in a tab at all
		if ($tabContent.length === 0 || $tabContent.hasClass('active')) {
			initMangaSlider($this);
		}
	});
