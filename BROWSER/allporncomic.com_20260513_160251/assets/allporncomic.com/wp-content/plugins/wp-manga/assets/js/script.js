...
jQuery(document).ready(function ($) {
	...
	*/
	...
	var $container = $('.wp-manga-single-manga');
		
	// Store original form position
	var originalFormPosition = null;
	
	// Handle reply button clicks
	$container.on('click', '.comment-reply-link', function(e) {
		e.preventDefault();
		e.stopPropagation();
		
		var $replyLink = $(this);
		var commentId = $replyLink.data('commentid');
		var $commentItem = $replyLink.closest('li.comment');
		var $commentBody = $replyLink.closest('.comment-body');
		var $commentForm = $container.find('#respond');
		
		if ($commentForm.length === 0) {
			$commentForm = $container.find('.comment-respond');
		}
		...
		if (!originalFormPosition) {
			originalFormPosition = $commentForm.parent();
		}
		
		// Update comment_parent hidden field
		var $commentParentField = $commentForm.find('input[name="comment_parent"]');
		if ($commentParentField.length === 0) {
			// Create hidden field if it doesn't exist
			var $form = $commentForm.find('form');
			if ($form.length === 0) {
				$form = $commentForm.find('.comment-form');
			}
			if ($form.length > 0) {
				$form.append('<input type="hidden" name="comment_parent" id="comment_parent" value="' + commentId + '">');
			}
		} else {
			$commentParentField.val(commentId);
		}
		
		// Move comment form after the comment item being replied to
		$commentForm.detach();
		...
		// Add reply mode class
		$commentForm.addClass('reply-mode');
	});
	
	// Handle cancel reply link
	$container.on('click', '.comment-reply-title small a', function(e) {
		e.preventDefault();
		e.stopPropagation();
		...
		$commentForm.find('input[name="comment_parent"]').val('0');
		
		// Move form back to original position
		if (originalFormPosition && originalFormPosition.length > 0) {
			$commentForm.detach();
			originalFormPosition.append($commentForm);
		} else {
			// Fallback: move after comments list
			var $commentsList = $container.find('.commentlist');
			if ($commentsList.length > 0) {
				$commentForm.detach();
				$commentsList.parent().append($commentForm);
			}
		}
		
		// Remove reply mode class
		$commentForm.removeClass('reply-mode');
