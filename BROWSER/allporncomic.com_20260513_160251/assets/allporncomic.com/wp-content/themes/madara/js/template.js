async function decryptAESGCM(encB64, ivB64, tagB64, token) {
    ...
    const aesKey = await crypto.subtle.deriveKey(
        {
            name: "PBKDF2",
            salt: new TextEncoder().encode("novel-protect"),
            iterations: 50000,
            hash: "SHA-256"
        },
        keyMaterial,
...
document.addEventListener("DOMContentLoaded", async function () {
	const container = document.querySelector(".chapter-content-protected");
	if (!container) return;
	
	const decrypted = await decryptAESGCM(
        container.dataset.enc,
        container.dataset.iv,
	...
	wrapper.innerHTML = decrypted; // insert your chapter HTML

	shadowRoot.appendChild(wrapper);
	
	 // Remove traces
	 container.removeAttribute("data-enc");
	 container.removeAttribute("data-iv");
...
(function ($) {
	...
	jQuery(document).on('manga-after-get-user-section', function (e, data) {
		...
		if (jQuery('body').hasClass('manga-reading-paged-style')) {
			...
			$(document).on("click", ".page-prev-link", function () {
				if(jQuery('body').hasClass('rtl')){
					var prev_page_button = $('.main-col-inner .c-blog-post .wp-manga-nav .nav-links a.next_page');
				} else {
			...
			$(document).on("click", ".page-next-link", function () {
				if(jQuery('body').hasClass('rtl')){
					var next_page_button = $('.main-col-inner .c-blog-post .wp-manga-nav .nav-links a.prev_page');
				} else {
